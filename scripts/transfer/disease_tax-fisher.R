...
disease.summ= read.csv("overlap/species_ds.csv")
rownames(disease.summ) = disease.summ$Species_rep
disease.summ = disease.summ[which(disease.summ$DS != 0),]
disease.summ$Classification = ifelse(disease.summ$DS > 0, "Disease", "Health")
metadata = disease.summ[,-1]
...
# check correlation
cor.df = as.data.frame.matrix(table(disease.summ[,c(rank, "Classification")]))
...
for (variable in var.list){
  ...
  pos_prop = cont.table["Disease",variable]/(cont.table["Disease","all_other"]+cont.table["Disease",variable])*100
  neg_prop = cont.table["Health",variable]/(cont.table["Health","all_other"]+cont.table["Health",variable])*100
...
result.df$Classification = ifelse(result.df$result > 0, "Disease", "Health")
rownames(result.df) = NULL

# bar plots
barplot.df = reshape2::melt(result.df)
barplot.df$variable = gsub("f__", "", barplot.df$variable)
...
bar.plot = ggplot(barplot.df, aes(x = reorder(variable, value), y = value, fill = type))+
  ...
  scale_fill_manual(values = c("tomato", "steelblue"), name="Classification", labels=c("Disease", "Health")) +
  coord_flip() +
  theme_classic() +
...
ggsave(file = "figures/tax_fisher.pdf", dpi=300, height=5, width=4)
