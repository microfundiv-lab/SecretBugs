...
keystone = read.delim("healthy_analysis/keystone_mean.tsv")
keystone = keystone[,c("Species_rep", "mean_centrality", "Status")]
prevalence = read.delim("healthy_analysis/prevalence_mean.tsv")
prevalence = prevalence[,c("Species_rep", "mean_prevalence")]
key.core = merge(keystone, prevalence, by="Species_rep")
...
# define keystone and core
key.core = key.core[which(key.core$mean_prevalence != 0 | !is.na(key.core$mean_centrality)),]
key.core$Key_class = ifelse(key.core$mean_centrality > key_thresh, "Keystone", NA)
key.core$Core_class = ifelse(key.core$mean_prevalence > core_thresh, "Core", NA)
key.df = key.core[,c("Species_rep", "Key_class")]
key.df = key.df[which(!is.na(key.df$Key_class)),]
...
upset.df = all.matrix.meta[,c("Core", "Disease", "Health", "Keystone", "Status", "Genus", "Family", "Species")]
