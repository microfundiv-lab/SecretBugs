...
# create df of props
pos_thresh = quantile(comb.agg$correlation, 0.9)
...
write.table(top.pos, file="top-pos-10_cag-170.txt", row.names=FALSE, col.names=FALSE, quote=FALSE)

top.neg = unique(comb.agg[which(comb.agg$correlation < neg_thresh),"neighbour"])
write.table(top.neg, file="top-neg-10_cag-170.txt", row.names=FALSE, col.names=FALSE, quote=FALSE)